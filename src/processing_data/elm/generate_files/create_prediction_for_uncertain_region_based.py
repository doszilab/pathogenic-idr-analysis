...
def make_prediction(clinvar_disorder,sequence_df,cut_off=0.5):
    ...
    predicted_region_with_am_info_all = predicted_region_with_am_info_all.merge(sequence_df[["Protein_ID","Sequence"]], on="Protein_ID", how="left")
    ...
    motif_metrics_df.to_csv(f"{files}/elm/clinvar/roi/roi_with_clinvar_uncertain.tsv", sep='\t', index=False)

    result_df = create_position_based_motif_df(predicted_region_with_am_info_all, clinvar_disorder)
    result_df.to_csv(f"{files}/elm/clinvar/roi/clinvar_uncertain_with_roi_overlaps.tsv", sep='\t', index=False)
    ...
    motif_metrics_df = create_clinvar_uncertain_motif_pred(clinvar_disorder_pathogenic, predicted_region_with_am_info_all, cut_off, files)

    motif_metrics_df.to_csv(f"{files}/elm/clinvar/roi/roi_with_clinvar_pathogenic", sep='\t', index=False)

    result_df = create_position_based_motif_df(predicted_region_with_am_info_all, clinvar_disorder_pathogenic)
    print(result_df)
    result_df.to_csv(f"{files}/elm/clinvar/roi/clinvar_pathogenic_with_roi_overlaps", sep='\t', index=False)
