...
def generate_pairs(order_table_df,elm_df,elm_interactor_domains,pfam_table,elm_type="known",mutation_type='pathogenic',disease_based=True):
            ...
            for index, motifrow in elm_with_mutations.iterrows():
                ...
                if not disease_mutations.empty:
                    ...
                    for protein_id_2 in proteins:
                        ...
                        # current_domain.loc[:, "Region"] = current_domain['Start'].astype(str) + "-" + current_domain[
                        #     'End'].astype(str)

                        # current_domain["Region"] = current_domain['Start'].astype(str) + "-" + current_domain['End'].astype(str)
...
if __name__ == '__main__':
    ...
    mutation_table_order_pathogenic = os.path.join(base_mut_dir,'Pathogenic', 'order', 'positional_clinvar_functional_categorized_final.tsv')
    mutation_table_order_uncertain = os.path.join(base_mut_dir,'Uncertain', 'order', 'positional_clinvar_functional_categorized_final.tsv')
    ...
    elm_known_predicted_pathogenic_df = pd.read_csv(elm_known_uncertain, sep='\t')
    elm_predicted_predicted_pathogenic_df = pd.read_csv(elm_predicted_uncertain, sep='\t')


    # Generate Tables
    generate_pairs(mutation_table_order_pathogenic_df, elm_known_pathogenic_df, elm_interactor_domains, pfam_table, elm_type="known",
                   mutation_type='pathogenic')
    generate_pairs(mutation_table_order_pathogenic_df, elm_predicted_pathogenic_df, elm_interactor_domains, pfam_table, elm_type="predicted",
                   mutation_type='pathogenic')

    # Generate Tables for Disease Classes
    generate_pairs(mutation_table_order_pathogenic_df, elm_known_pathogenic_df, elm_interactor_domains, pfam_table, elm_type="known",
                   mutation_type='pathogenic',disease_based=False)
    generate_pairs(mutation_table_order_pathogenic_df, elm_predicted_pathogenic_df, elm_interactor_domains, pfam_table, elm_type="predicted",
                   mutation_type='pathogenic',disease_based=False)


    # Uncertain (Ordered Uncertain + Pathogenic)
    order_uncertain_and_pathogenic = pd.concat([mutation_table_order_pathogenic_df,mutation_table_order_uncertain_df],ignore_index=True)

    generate_pairs(order_uncertain_and_pathogenic, elm_known_uncertain_df, elm_interactor_domains, pfam_table, elm_type="known",
                   mutation_type='uncertain')
    generate_pairs(order_uncertain_and_pathogenic, elm_predicted_uncertain_df, elm_interactor_domains, pfam_table,
                   elm_type="predicted",
                   mutation_type='uncertain')

    # # Generate Tables for Disease Classes
    generate_pairs(order_uncertain_and_pathogenic, elm_known_uncertain_df, elm_interactor_domains, pfam_table, elm_type="known",
                   mutation_type='uncertain',disease_based=False)
    generate_pairs(order_uncertain_and_pathogenic, elm_predicted_uncertain_df, elm_interactor_domains, pfam_table,
                   elm_type="predicted",
                   mutation_type='uncertain',disease_based=False)
    ...
    generate_pairs(order_predicted_pathogenic_and_pathogenic, elm_known_predicted_pathogenic_df, elm_interactor_domains, pfam_table, elm_type="known",
                   mutation_type='predicted_pathogenic')
