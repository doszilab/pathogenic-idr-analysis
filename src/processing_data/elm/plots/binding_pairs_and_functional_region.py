...
def plot_motif_counts_distribution(relevant_regions_df,fig_dir=None):
    """
    ...
    motif_counts_distribution.plot(kind='bar',color=main_color)
    plt.title('PIP Region Counts per Protein')
    plt.xlabel('Number of PIP Region')
    plt.ylabel('Number of Proteins')
    plt.xticks(rotation=90)
    plt.tight_layout()
    if fig_dir:
        plt.savefig(f"{fig_dir}/motif_count_per_protein.png", dpi=300, bbox_inches='tight')
    plt.show()


def plot_motifs_by_length_distribution(relevant_regions_df,fig_dir=None,max_length=30):
    """
    Plots the distribution of predicted motifs by their length.
    """
    # Optionally, group by region length
    motifs_by_length = relevant_regions_df['Length']

    lengths_capped = motifs_by_length.apply(lambda x: x if x < max_length else max_length)

    # Count the distribution of motif lengths after capping
    motifs_by_length_distribution = lengths_capped.value_counts().sort_index()

    # Rename the bin for capped lengths (i.e., length 20) to '20+'
    if max_length in motifs_by_length_distribution.index:
        motifs_by_length_distribution.rename(index={max_length: f'{max_length}+'}, inplace=True)

    # Plot the distribution of motifs by their length
    plt.figure(figsize=(5, 4))
    motifs_by_length_distribution.plot(kind='bar',color=main_color)
    plt.title('PIP Region Length Distribution')
    plt.xlabel('PIP Length')
    plt.ylabel('Number of PIP')
    plt.xticks(rotation=90)
    plt.tight_layout()
    if fig_dir:
        plt.savefig(f"{fig_dir}/motif_length.png", dpi=300, bbox_inches='tight')
    plt.show()
...
def plot_combined_disorder_pie_charts(
    protein_regions, relevant_regions_df, am_disorder_df, functional_color="#FF9999", non_functional_color="#66B2FF"
):
    """
    Combines two pie charts:
    1. Percentage of disordered regions with motifs.
    2. Proportion of predicted motif positions.
    """
    # Initialize counts for the first pie chart
    total_disordered_regions = 0
    disordered_regions_with_motifs = 0
    ...
    for protein, data in tqdm(protein_regions.items()):
        ...
        if total_disordered_regions == 100:
            break
    ...
    # Calculations for the second pie chart
    relevant_region_positions = relevant_regions_df["Length"].sum()
    total_positions = am_disorder_df.shape[0]
    non_relevant_positions = total_positions - relevant_region_positions
    ...
    sizes2 = [relevant_region_positions, non_relevant_positions]
    ...
    plt.suptitle("Proportion of Predicted Motif in Disordered Regions ")
...
def plot_combined_disorder_donut_chart(
    protein_regions, relevant_regions_df, am_disorder_df, functional_color="#FF9999", non_functional_color="#66B2FF",fig_dir=None
):
    """
    ...
    for protein, data in tqdm(protein_regions.items()):
        ...
        # print(disordered_regions_with_motifs)
        # if disordered_regions_with_motifs >= 100:
        #     break
    ...
    data1 = {
        'Regions': ['PIPs', 'Non-PIPs'],
        'Count': [disordered_regions_with_motifs, disordered_regions_without_motifs]
    }
    percentage_df = pd.DataFrame(data1)

    # Calculations for the second donut chart
    relevant_region_positions = relevant_regions_df["Length"].sum()
    total_positions = am_disorder_df.shape[0]
    non_relevant_positions = total_positions - relevant_region_positions
    ...
    # Add a shared title
    plt.suptitle("PIP Region in IDRs")

    # Add a common legend
    fig.legend(
        handles=[
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=functional_color, markersize=10, label='PIPs'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=non_functional_color, markersize=10, label='Non-PIPs')
        ],
...
def plot_elm_count_of_counts(df, fig_dir=None):
    ...
    labels = ['1-10','10-25', '25-50', '50-100', '100-200', '200+']
...
if __name__ == "__main__":
    ...
    main_color = '#f08080'
    functional_color = 'red'
    non_functional_color = 'grey'

    base_dir = '/dlab/home/norbi/paper_projects/IDP_Pathogenic_Mutations'
    # base_dir = 'D:\PycharmProjects\IDP_Pathogenic_Mutations'

    fig_dir = os.path.join(base_dir,'plots/sm/fig2')
    ...
    # Generate the plots
    plot_motif_counts_distribution(relevant_regions_df, fig_dir=fig_dir)
    plot_motifs_by_length_distribution(relevant_regions_df, fig_dir=fig_dir)
    # exit()
    # print(relevant_regions_df['Protein_ID'].nunique())
    # print(relevant_regions_df.shape[0])
    # print(region_df.shape[0])
    # plot_combined_disorder_pie_charts(protein_regions, relevant_regions_df, am_disorder_df)
    plot_combined_disorder_donut_chart(protein_regions, relevant_regions_df, am_disorder_df, fig_dir=fig_dir)
    # exit()

    # elm_predicted_regions_path = f"{files}/elm_predicted_with_am_info_all_disorder_with_rules.tsv"
    elm_predicted_regions_path = f"{files}/elm_predicted_disorder_corrected.tsv"
    elm_known_regions_path = f"{files}/elm_known_with_am_info_all_disorder_with_rules.tsv"

    elm_known_disorder = pd.read_csv(elm_known_regions_path, sep='\t')
    elm_predicted_disorder = pd.read_csv(elm_predicted_regions_path, sep='\t')

    elm_sequential = elm_predicted_disorder[elm_predicted_disorder['Sequential_Rule']]

    plot_elm_types(elm_sequential,fig_dir=fig_dir)
    # exit()
    plot_elm_count_of_counts(elm_known_disorder,fig_dir=fig_dir)
    # print(elm_sequential)
    plot_elm_count_of_counts(elm_sequential,fig_dir=fig_dir)
    exit()
