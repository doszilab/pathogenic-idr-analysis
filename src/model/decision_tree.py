...
def main_prediction_conservation(known_df, predicted_df, save=False):
    """
    Kifejezetten a konzerváltság alapú adatokhoz készült egyszerűsített modell tanítás és mentés.
    """
    df = pd.concat([predicted_df, known_df])

    # Mivel a konzerváltsági fájlokban is ezeket az oszlopneveket használod a filter_df-ben:
    metrics_cols = [
        'motif_am_mean_score',
        'key_residue_am_mean_score',
        'flanking_residue_am_mean_score',
    ...
    n_boostrap = 100
    all_scores = []
    for i in tqdm(range(1, n_boostrap), desc="Conservation Bootstrap"):
        sampled_neg = np.random.choice(idx_neg, size=len(idx_pos))
        sampled_indices = np.concatenate([idx_pos, sampled_neg])
        X_sample = X[sampled_indices]
        y_sample = y[sampled_indices]
    ...
    print(f"Mean accuracy (Conservation): {np.mean(all_scores):.3f} ± {np.std(all_scores):.3f}")
    ...
    df_scores.boxplot(column="Accuracy", vert=False)
    plt.title(f"Cross-validation Accuracies ({cv}-fold, {n_boostrap} bootstrap)")
    plt.savefig("/dlab/home/norbi/paper_projects/IDP_Pathogenic_Mutations/plots/sm/decision_tree/accuracy_conservation.png", dpi=300)
    plt.close()

    # Modell tanítása a teljes konzerváltsági adaton
    clf.fit(X, y)

    # Predikciók rögzítése
    predictions = clf.predict(X)
    ...
    # Mentés külön mappába
    if save:
        cons_save_dir = os.path.join(base_dir, 'decision_tree_conservation')
        if not os.path.exists(cons_save_dir):
            os.makedirs(cons_save_dir)

        save_path = os.path.join(cons_save_dir, 'elm_conservation_results.tsv')
        df.to_csv(save_path, sep='\t', index=False)
        print(f"Results saved to: {save_path}")

    # Fa vizualizáció mentése (hogy lásd a különbséget)
    plt.figure(figsize=(15, 4))
    tree.plot_tree(clf, filled=True, feature_names=metrics_cols, fontsize=6)
    plt.savefig(os.path.join(plot_dir, "tree_conservation.png"), dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    # Conservation
    df_predicted_cons = filter_df(pd.read_csv(
        os.path.join(base_dir, "conservation", 'elm_predicted.tsv'),
        sep='\t'
    ))
    df_known_cons = filter_df(pd.read_csv(
        os.path.join(base_dir, "conservation", 'elm_known.tsv'),
        sep='\t'
    ))

    main_prediction_conservation(df_known_cons, df_predicted_cons, save=True)
