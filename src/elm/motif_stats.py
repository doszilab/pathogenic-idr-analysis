...
def plot_totals_subplots(known_labels, known_total_values, known_color,
                         predicted_labels, predicted_total_values, predicted_color, figsize=(8, 6),plot_dir=None,conservation=""):
    ...
    plt.tight_layout()
    if plot_dir:
        plt.savefig(os.path.join(plot_dir,f"{title}{conservation}.png"))
    plt.show()
...
def plot_motif_and_position(
        labels, motif_values,
        position_values,
        known_color,figsize=(6, 6),title="Pathogenic Mutations in ELM",plot_dir=None,conservation=""):
...
def plot_percentages_bars(info_dct_known, info_dct_known_pip, known_total_numbers,
                          choosen_motif='PIP Motifs', plot_dir=None,conservation=""):
    clinvar_position = 'Total ClinVar Position in ELM Regions'
    elm_with_clinvar = 'ELM Motifs with ClinVar Position'
...
def plot_number_of_elms(plot_dir):
    ...
    regions = {
        ...
        "DecisionTreeConservation": 'elm/decision_tree_conservation/elm_conservation_results.tsv',
    ...
    names = {
        ...
        'PEM':'PEM Motifs',
        'DecisionTree':'AMPEM',
        'DecisionTreeConservation':'Conservation',
        "SlimPrint":"SlimPrint",
    }
    ...
    for name, regions in lst:
        for method in method_names:
            ...
            conservation = " Conservation" if "Conservation" in method else ""
            ...
            if method == "DecisionTree" or method == "DecisionTreeConservation" :
            ...
            plot_percentages_bars(info_dct_known,info_dct_known_filtered,known_total_numbers,choosen_motif=method_name,plot_dir=plot_dir,conservation=conservation)

            # Figure 5 Motif Retention
            plot_totals_subplots(known_labels, known_total_values, known_color,
                                 predicted_labels, predicted_total_values, predicted_color, figsize=(4, 4),plot_dir=plot_dir,conservation=conservation)
            ...
            plot_motif_and_position(known_labels,known_motif_values,known_pos_values,known_color, figsize=(4, 4),plot_dir=plot_dir,conservation=conservation)
            continue
